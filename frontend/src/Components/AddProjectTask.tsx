...
const AddProjectTasks = ({ taskId, onTaskUpdate, inputContent, show, handleShow }: PropTypes) => {
  ...
  const onSubmit = async (data: InitialState) => {
    ...
    try {
      ...
      if (taskId) {
        await fetch(`${api_url}/projectTask/${taskId}`, {
          method: 'PUT',
          headers: {
        ...
        await fetch(`${api_url}/projectTask`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${results}`,
...
export default AddProjectTasks;